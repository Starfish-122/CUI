...
// 경로 그룹 및 개인 폴더 패턴
const isRouteGroup = (path) => /^\([^)]+\)$/.test(path);
const isPrivateFolder = (path) => path.startsWith('_');

/**
 * 라우트 이름을 사용자 친화적인 텍스트로 변환
 * @param {string} route - 라우트 이름 (폴더명)
...
/**
 * 네비게이션 링크 목록 생성
 * @param {Array} routes - app 폴더 내의 라우트 목록
